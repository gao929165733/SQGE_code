...
class Framework:
    def __init__(self,
        ...
        self.device = device
        self.opt = opt
        self.tokenizer = BertTokenizer.from_pretrained("./pretrain_models/bert-base-uncased")
    ...
    def evaluate(self,
        ...
        for i in tqdm(range(eval_epoch)):
            ...
            with torch.no_grad():
                ...
                loss, logits, pred, support_emb, query_emb, prototype = model(support_set, query_set, evalN, K, Q, O, train_step)
            self.metric.update_state(pred, query_set['entity_label'], id2label, query_emb, prototype)
            # 打印结果
            """
            if i == 6:
    ...
    def train(self,
        ...
        for epoch in range(train_epoch):
            ...
            with autocast():
                loss, logits, pred, _, _, _ = model(support_set, query_set, trainN, K, Q, O, epoch)
                loss = loss.mean()
